...
Indicateurs visuels
- Infecté: anneau orange autour de la cellule.
- Rétabli: anneau vert discret.
- Immunisé: pas d’indicateur dédié (évite la confusion et la surcharge).
...
## Infection & Toxines (mise à jour)

- Proximité des toxines: drain d’énergie supplémentaire par tick, proportionnel à la force (`toxinProximityDrain * strength`).
- Pénalité de digestion: les herbivores ne récupèrent qu’une fraction de l’énergie des nourritures en zone toxique (`toxinDigestMultiplier`).
- Contamination: probabilité par tick en zone toxique (`toxinInfectProb`), durée d’infection (`infectionDuration`), blocage repro (`infectionReproBlockDuration`).
- Drain infectieux: `infectionExtraDrain` par tick en plus du métabolisme.
- Immunité: temporaire après guérison (`infectionImmunityDuration`); `infectionNaturalImmunityRate` pour l’immunité innée.
- Propagation douce: rayon `infectionTransmitRadius`, probabilité liée à `infectionR0`, limitation via `infectionSpreadCooldown`.
